...
class CupsPrintConnector implements PrintConnector
{
    ...
    public function __construct($dest)
    {
        ...
        if (count($valid) == 0) {
            Notifier::Add("You do not have any printers installed on " .
            "this system via CUPS. Check 'lpr -a'.", Notifier::NOTIFIER_INFO);
        ...
        if (array_search($dest, $valid, true) === false) {
            Notifier::Add("'$dest' is not a printer on this system. " .
            "Printers are: [" . implode(", ", $valid) . "]", Notifier::NOTIFIER_ERROR);
     ...
     * Send job to printer.
     */
    public function finalize($copies = 1)
    {
    ...
    protected function getCmdOutput($cmd)
    {
        ...
        if ($retval != 0) {
            ...
            Notifier::Add("Command $cmd failed: $errorStr", Notifier::NOTIFIER_ERROR);
        }
        return $outputStr;
    }
